...
#define FT5X0X_NAME	              "ft5x0x_ts"//"synaptics_i2c_rmi"//"synaptics-rmi-ts"// ft5406_ts ft5x06_ts ft5x0x
...
printk("[ELDIAU] ft5406_ts.c: fts_i2c_txdata\n");
...
printk("[ELDIAU] ft5406_ts.c: i2c_write_interface\n");
...
printk("[ELDIAU] ft5406_ts.c: i2c_read_interface\n");
...
printk("[ELDIAU] ft5406_ts.c: fts_register_read\n");
...
printk("[ELDIAU] ft5406_ts.c: fts_ctpm_fw_upgrade\n");
...
printk("[ELDIAU] ft5406_ts.c: fts_ctpm_fw_upgrade_with_i_file\n");
...
{
	int ret;
printk("[ELDIAU] ft5406_ts.c: ft5406_read_regs\n");
...
{
	int ret; 
printk("[ELDIAU] ft5406_ts.c: ft5406_set_regs\n");
...
printk("[ELDIAU] ft5406_ts.c: ft5406_queue_work\n");
...
printk("[ELDIAU] ft5406_ts.c: ft5406_interrupt\n");
...
printk("[ELDIAU] ft5406_ts.c: ft5406_suspend\n");
...
printk("[ELDIAU] ft5406_ts.c: ft5406_resume\n");
...
{
printk("[ELDIAU] ft5406_ts.c: ft5406_suspend_early\n");
...
{
printk("[ELDIAU] ft5406_ts.c: ft5406_resume_early\n");
...
printk("[ELDIAU] ft5406_ts.c: ft5406_remove\n");
...
printk("[ELDIAU] ft5406_ts.c: ft5406_probe\n");
...
/*ELDIAU*/
//enable_irq(ft5x0x_ts->irq);
// da arch/arm/mach-rk29/include/mach/gpio.h
//#define PIN_BASE                0
//#define NUM_GROUP               32
//#define	RK29_PIN0_PA2		(0*NUM_GROUP + PIN_BASE + 2)   // IRQ 102  forse si chiama GPIO6_B[7]  QUESTO e' SICURO
//#define	RK29_PIN6_PC3		(6*NUM_GROUP + PIN_BASE + 19)  // IRQ 311  forse si chiama GPIO6_A[2]
//IRQ non suato ??? 163:          0        GPIO  <NULL> - 100 = 63 = 32 + 31 #define RK29_PIN1_PD7 GPIO3_A[0]/I2S1_CLK

// IRQ 41 parla ma non lo legge nessun driver
...
//}
//#define NR_AIC_IRQS     (IRQ_A8IRQ3+1) // = 100 
//#define IRQ_A8IRQ3      RK29XX_IRQ(67) // = 99
...
/*printk("[ELDIAU] switch board on\n");*/

/*    if(gpio_request(TOUCH_RESET_PIN,"ft5x0x-touch-reset-enable") != 0) {*/
/*      gpio_free(TOUCH_RESET_PIN);*/
/*      printk("failed to allocate TOUCH_RESET_PIN\n");*/
/*      goto exit_input_dev_alloc_failed;*/
/*    }*/

/*    if(gpio_request(TOUCH_INT_PIN,"ft5x0x-touch-int-enable") != 0) {*/
/*      gpio_free(TOUCH_INT_PIN);*/
/*      printk("failed to allocate TOUCH_INT_PIN\n");*/
/*      goto exit_input_dev_alloc_failed;*/
/*    }*/
...
/*    msleep(350);*/
    ...
    #define TOUCH_RESET_PIN RK29_PIN0_PA2
    #define TOUCH_INT_PIN RK29_PIN6_PA6
    gpio_request(TOUCH_RESET_PIN,NULL);
    gpio_request(TOUCH_INT_PIN,NULL);
...
/*    auOEMVoodoos[2] = 0x01;*/
/*    auOEMVoodoos[3] = 0xC1;*/
/*    //if ((err=fts_i2c_txdata(auOEMVoodoos, sizeof(auOEMVoodoos))) < 0)*/
/*    if ((err=fts_i2c_txdata(auOEMVoodoos, 4)) < 0)*/
/*    {*/
/*        printk("[ELDIAU]attempt to activate ctpm device *work-mode with voodoos failed with err[%d]\n", err);*/
/*        goto exit_irq_request_fail;*/
/*    }*/
/*    msleep(20);*/

/*for(gpionum = 0; gpionum < 224; gpionum++)*/
/*{*/

/*    printk("[ELDIAU] ft5406_ts.c: trying to wake up on PIN %d\n", gpionum);*/

/*    gpio_request(gpionum, "ft5406 power-up");*/
...
/*		if(ret > 0) { printk("[ELDIAU] ft5406_ts.c: ret=%d\n", ret); break;}*/
/*		retry++;*/
/*	}*/
/*    if(ret > 0) { printk("[ELDIAU] ft5406_ts.c: wake up done ON !!!!\n"); break; }*/

/*    printk("[ELDIAU] ft5406_ts.c: ret=%d\n", ret);*/
...
/*	  }*/
	...
	printk("[ELDIAU]  ft5406_ts.c: client->dev.driver->name %s  ,%d \n",client->dev.driver->name,ft5x0x_ts->irq);
	...
	buf_w[0] = 6;
    printk("[ELDIAU] ft5406_ts.c: write buf[0x88] = %d\n", buf_r[0]);
	err = ft5406_set_regs(client,0x88,buf_w,1);
	buf_r[0] = 0;
	err = ft5406_read_regs(client,0x88,buf_r,1);
	printk("[ELDIAU] ft5406_ts.c: read buf[0x88] = %d\n", buf_r[0]);
    return 0;
...
{
printk("[ELDIAU] ft5406_ts.c: ft5406_ts_init\n");
	return i2c_add_driver(&ft5406_driver);
}

static void __exit ft5406_ts_exit(void)
{
printk("[ELDIAU] ft5406_ts.c: ft5406_ts_exit\n");
